...
rem ***********************************
rem Description: Installs sqldeveloper-4.1.2.20.64-no-jre
rem ***********************************
...
>"%logfile%" (
	...
	%~dp0unzip -q %~dp0sqldeveloper-4.1.2.20.64-no-jre.zip -d C:\Oracle\product 2>&1 && (
		@echo INFO: %~dp0sqldeveloper-4.1.2.20.64-no-jre.zip unzipped to C:\Oracle\product.
	) || (
		@echo ERR:  %~dp0unzip -q %~dp0sqldeveloper-4.1.2.20.64-no-jre.zip -d C:\Oracle\product failed.
		GOTO :ERR
	)
	...
	if not exist "C:\Program Files (x86)\Java\jdk1.8.0_65" %~dp0jdk-8u65-windows-i586.exe /s 2>&1 && (
		@echo INFO: %~dp0jdk-8u65-windows-i586.exe installed Successfully.
	) || (
		@echo ERR:  %~dp0jdk-8u65-windows-i586.exe failed.
		GOTO :ERR
	)
